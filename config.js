...
function applySQLiteResilience(sequelizeInstance) {
  if (!sequelizeInstance || sequelizeInstance.__sqliteGuardsApplied) return;
  ...
  const flushQueue = async () => {
    if (queueActive || writeQueue.length === 0) return;
    queueActive = true;
  ...
  const isWriteQuery = (sql) => {
    ...
    return ["INSERT", "UPDATE", "DELETE", "CREATE", "ALTER", "DROP", "PRAGMA"].some(k => normalizedSql.startsWith(k));
  };

  sequelizeInstance.query = function serializedQuery(sql, ...rest) {
    if (!isWriteQuery(sql)) return originalQuery(sql, ...rest);
    return new Promise((resolve, reject) => {
      writeQueue.push({ task: () => originalQuery(sql, ...rest), resolve, reject });
...
const baseConfig = {
  ...
  PLATFORM: isHeroku ? "Heroku" : isKoyeb ? "Koyeb" : isRailway ? "Railway" : "Other server",
  ...
  BOT_INFO: process.env.BOT_INFO || "Caleb AchekBot;Calebosky;default",
  RBG_KEY: process.env.RBG_KEY || "",
  ALLOWED: process.env.ALLOWED || "91,94,2",
  NOT_ALLOWED: process.env.NOT_ALLOWED || "852",
  CHATBOT: process.env.CHATBOT || "off",
  HANDLERS: process.env.HANDLERS || ".,",
  STICKER_DATA: process.env.STICKER_DATA || "Caleb AchekBot",
  BOT_NAME: process.env.BOT_NAME || "Caleb AchekBot",
  AUDIO_DATA: process.env.AUDIO_DATA || "default",
...
const config = new Proxy(baseConfig, {
  get(target, prop) {
    ...
    if (key === "inspect" || key === Symbol.for("nodejs.util.inspect.custom")) return () => ({ ...target, ...Object.fromEntries(dynamicValues) });
    if (key === 'isPrivate') return (dynamicValues.get('MODE') || target.MODE) === 'private';
    if (dynamicValues.has(key)) return dynamicValues.get(key);
    return target[key] || process.env[key];
  },
  set(target, prop, value) {
    dynamicValues.set(prop, value);
    return true;
  }
});
...
Object.defineProperty(config, "loadFromDB", {
  value: function (dbValues = {}) {
    let loadedCount = 0;
    const booleanKeys = [...settingsMenu.map((item) => item.env_var), "MANGLISH_CHATBOT"];
    for (const [key, value] of Object.entries(dbValues)) {
      if (value !== undefined && value !== null) {
        this[key] = booleanKeys.includes(key) ? convertToBool(value) : value;
        loadedCount++;
      }
    }
